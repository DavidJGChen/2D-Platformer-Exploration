...
public class ActorController : MonoBehaviour
{
    ...
    private void DescendSlope(ref Vector2 deltaMove) {
        ...
        if (hit) {
            ...
            if (slopeAngle == 0 || slopeAngle >= maxSlopeAngle || Mathf.Sign(hit.normal.x) != dirX) return;
        ...
        if (leftHit || rightHit) {
            ...
            if (slopeAngle > maxSlopeAngle) {
                ...
                if (Mathf.Sign(deltaMove.x) != Mathf.Sign(dX) || Mathf.Abs(deltaMove.x) < Mathf.Abs(dX)) {
                    ...
                    Vector2 DELETE = deltaMove.normalized * (mag + skinWidth);
                    Color c = Color.cyan;

                    hit = Physics2D.Raycast(rayOrigin, deltaMove.normalized, mag + skinWidthAngled, collisionMask);
                    if (hit) {
                        c = Color.red;
                    ...
                    Debug.DrawRay(rayOrigin, DELETE, c);
                }
            }
        }
    }
